...
1. **文件結構建立** ✅
   - 建立 `docs/development/` 目錄
   - 建立 `docs/integration/` 目錄
   - 建立 `docs/guides/` 目錄

2. **檔案移動** ✅
   - 將 `.cursor/*.md` 移到 `docs/development/` (11 個檔案)
   - 將 `APPLICATIONS_INTEGRATION_SUMMARY.md` 移到 `docs/integration/`
   - 將 `STATUS_LABELS_SUMMARY.md` 移到 `docs/integration/`
   - 將 `STORAGE_FIX_GUIDE.md` 移到 `docs/guides/`

3. **Demo 頁面整理** ✅
   - 建立 `app/demo/` 目錄
   - 移動 `career-modal-demo/` 到 `app/demo/`
   - 移動 `form-demo/` 到 `app/demo/`
   - 移動 `expanded-salary-demo/` 到 `app/demo/`
   - 移動 `modal-style-demo/` 到 `app/demo/`
   - 移動 `app/dashboard/test/` 到 `app/demo/test/`

4. **程式碼清理** ✅
   - 修正 `libs/gpt.ts` 的 `any` 類型為 `OpenAIMessage[]`
