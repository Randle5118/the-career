...
export const EducationSchema = z.object({
  ...
  comment: z.string().max(500).nullable().optional(), // 備考 (可為 null 或 undefined)
});
...
export const WorkExperienceSchema = z.object({
  ...
  positions: z.array(PositionSchema).optional(), // 役職履歴（可選）
...
export const ResumeFormDataSchema = z
  .object({
    // 履歷元資訊
    ...
    public_expires_at: z.string().nullable().optional(), // 公開到期日 - 改為寬鬆驗證，因為前端可能送來空字串或部分格式
...
export const PublishResumeSchema = z.object({
  ...
  public_url_slug: z
    ...
    .max(100)
    .optional(),
  public_expires_at: z.string().nullable().optional(), // 允許 ISO string 或 null
});
...
export const UpdatePublishedResumeSettingsSchema = z
  .object({
  ...
  .refine(
    (data) =>
      ...
      data.public_url_slug !== undefined ||
      data.public_expires_at !== undefined,
    {
...
export type UpdatePublishedResumeSettingsData = z.infer<
  typeof UpdatePublishedResumeSettingsSchema
>;
